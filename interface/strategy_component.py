...
class StrategyEditor(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self._add_buttom = tk.Button(self._commands_frame, text="Adicionar estratégia", font=GLOBAL_FONT,
                                    command=self._add_strategy_row, bg=BG_COLOR_2, fg=FG_COLOR)
        self._add_buttom.pack(side=tk.TOP)
        ...
        self._extra_params = {
            ...
            "Breakout": [
                {"code_name": "min_volume", "name": "Minimun Volume", "widget": tk.Entry, "data_type": float},
            ]
        }
    ...
    def _show_popup(self, b_index: int):
        ...
        self._popup_window = tk.Tk() # tk.Toplevel(self)
        self._popup_window.wm_title("Parâmetros")
        self._popup_window.resizable(False, False)
        ...
        start_selected = self.body_widgets['strategy_type_var'][b_index].get()

        row_nb = 0
        for param in self._extra_params[start_selected]:
        ...
        self._validation_buttom = tk.Button(self._popup_window, text="Validar", bg=BG_COLOR_2, fg=FG_COLOR,
                            command=lambda: self._validate_parameters(b_index))

        self._validation_buttom.grid(row=row_nb, column=0,columnspan=2)
